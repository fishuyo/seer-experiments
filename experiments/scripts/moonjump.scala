...
class Player(var id:Int, var color:Vec3) extends Entity {
	...
	override def collide(e:Entity):Unit = e match {
		case m:Moon =>
			if( attachedTo.isDefined && attachedTo.get == m) return
	...
	def doJump(){
		...
		velocity = m.velocity + n * 0.02 + t * groundVelocity*0.015
...
class Moon(size:Float) extends Entity {
	...
	override def init() = {
		...
		model.material.lightingMix = 0.5
		model.material.textureMix = 0.5
		inited = true
	}
}
