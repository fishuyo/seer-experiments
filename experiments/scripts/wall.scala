...
class Tree() extends Animatable {
  ...
  var root = TreeNode(None,1.0f,1.0f)

  val sLength = Randf(1.0f,1.0f,true)
  val sRatio = Randf(.8f,.8f,true)
  val bLength = Randf(1.0f,1.0f,true)
  val bRatio = Randf(.6f,.6f,true)
  val sThick = Randf(1.0f,1.0f,true)
  val taper = Randf(.9f,.9f,true)
  val sAngle = RandVec3( Vec3(0,0,0), Vec3(0,0,0), true)
  val bAngle = RandVec3( Vec3(0,10.0f.toRadians,0), Vec3(0,10.0f.toRadians,0), true)

  def set(r:Randf, s:Float=1.0f) = (v:Seq[Float]) => {r.set(v(0)*s); dirty=true}
  def setMin( r:Randf, s:Float=1.0f) = (v:Float) => {r.min = v*s; dirty=true}
  def setMax( r:Randf, s:Float=1.0f) = (v:Float) => {r.max = v*s; dirty=true}
  def setMinMax( r:Randf, s:Float=1.0f) = (v1:Float,v2:Float) => {r.min = v1*s; r.max = v2*s; dirty=true}
  var dirty = true
  ...
  val branchNum = Chooser(Array(0,1,2),Array(0.0f,0,1.0f))
...
object TreeNode {
  ...
  var taper=.8f
  var glprimitive = Model(Cylinder.generateMesh(1.0f,taper,10)) //Primitive3D.cylinder(Pose(),Vec3(1), 1.0f, taper, 10)
  
  // def apply( p:Pose=Pose(Vec3(0),Quat().fromEuler(Vec3(math.Pi/2,0,0))), d:Float=1.0f, t:Float=.2f, q:Quat=Quat() ) = new TreeNode {
  //  restPose = Pose(p);
  //  relQuat = q;
  //  pose = p;
  //  lPose = Pose(p);
  ...
  //   //k = 10.0f * thick*thick*thick / (4*dist*dist*dist)
  //   k = 10.0f * thick/ (4*dist)
  //  //glprimitive = GLPrimitive.cylinder(pose,Vec3(1,1,d), d*thick, d*thick*taper, 10)
  // } 

  def apply( p:Option[TreeNode], d:Float=1.0f, t:Float=.2f, rel:Quat=Quat() ) = new TreeNode(p){
...
object Script extends SeerScript {
  ...
  lquad.material.textureMix = 1.0f
  rquad.material.textureMix = 1.0f
  ...
  override def animate(dt:Float){
    ...
    if( dirty ){  // resize everything if using sub image
      ...
      val s1 = Vec3(1.0f,-(h/w.toFloat), 1.0f)
      val s2 = Vec3(-1.0f,-(h/w.toFloat), 1.0f)
      lquad.scale.set(s1)
      rquad.scale.set(s2)
      if(texture != null) texture.dispose
