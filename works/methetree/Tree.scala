...
class Tree() extends Animatable {
  ...
  val sLength = Randf(1f,1f,true)
  val sRatio = Randf(.8f,.8f,true)
  val bLength = Randf(1f,1f,true)
  val bRatio = Randf(.6f,.6f,true)
  val sThick = Randf(1f,1f,true)
  val taper = Randf(.9f,.9f,true)
  val sAngle = RandVec3( Vec3(0,0,0), Vec3(0,0,0), true)
  val bAngle = RandVec3( Vec3(0,10f.toRadians,0), Vec3(0,10f.toRadians,0), true)
  ...
  def set(r:Randf, s:Float=1f) = (v:Seq[Float]) => {r.set(v(0)*s); dirty=true}
  def setMin( r:Randf, s:Float=1f) = (v:Float) => {r.min = v*s; dirty=true}
  def setMax( r:Randf, s:Float=1f) = (v:Float) => {r.max = v*s; dirty=true}
  def setMinMax( r:Randf, s:Float=1f) = (v1:Float,v2:Float) => {r.min = v1*s; r.max = v2*s; dirty=true}
  var dirty = true
  ...
  val branchNum = Chooser(Array(0,1,2),Array(0f,1f,0f))
...
object TreeNode {
	...
	var taper=.8f
	var model = Model(Cylinder.generateMesh(1f,taper,10)) //Primitive3D.cylinder(Pose(),Vec3(1), 1f, taper, 10)

  def apply( p:Pose=Pose(Vec3(0),Quat().fromEuler(Vec3(-math.Pi/2,0,0))), d:Float=1f, t:Float=.2f, q:Quat=Quat() ) = new TreeNode {
  	restPose = Pose(p);
  	relQuat = q;
  	pose = p;
  	lPose = Pose(p);
    ...
    //k = 10f * thick*thick*thick / (4*dist*dist*dist)
    k = 10f * thick/ (4*dist)
  	//model = GLPrimitive.cylinder(pose,Vec3(1,1,d), d*thick, d*thick*taper, 10)
 	} 
  /*def apply( p:TreeNode, ang: Float, d: Float ) = {
    
    val v = Vec3( math.cos( ang * math.Pi/180f ), math.sin( ang * math.Pi/180f ), 0f ) * d + p.pos
...
class TreeNode extends Animatable {
  ...
  // var lPos = Vec3(0)
  var accel = Vec3(0)
  var euler = Vec3(0f,0f,0f)
  var lEuler = Vec3(0f,0f,0f)
  var mass = 1f
  var damp = 100f
  ...
  var w = Randf(0,3.14f)()
  var f = 1f/dist * 1f
  var k = 10f * thick*thick*thick / (dist*dist*dist)
  ...
  /*def branch( depth: Int, angle: Float=10f, ratio: Float=.9f, t:Int=0 ) : Int = {
