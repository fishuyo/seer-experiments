...
object Randf{
	val gen = new JRandom
	def apply(min:Float=0f, max:Float=1f, gaus:Boolean=false) = new Randf(min,max,gaus)
}

class Randf( var min:Float=0f, var max:Float=1f, var gaus:Boolean=false){

	def apply():Float = {
		if( min == max ) return min
		if( gaus ){
			(Randf.gen.nextGaussian.toFloat / (10f) + .5f) * (max-min) + min
		}else{
