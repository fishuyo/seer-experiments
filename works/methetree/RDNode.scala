...
class RDNode extends BackBufferNode {
  ...
  """)
  override def render(){
    renderer.shader.uniforms("brush") = Mouse.xy()
    renderer.shader.uniforms("width") = Window.width.toFloat
    renderer.shader.uniforms("height") = Window.height.toFloat
    // renderer.shader.uniforms("feed") = 0.037 //62
    // renderer.shader.uniforms("kill") = 0.06 //093
    // renderer.shader.uniforms("dt") = dt
    super.render()
  }
}