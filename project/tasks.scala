...
object Tasks {
  ...
  lazy val assembly = assemblyKey <<= (fullClasspath in Runtime, // dependency to make sure compile finished
    ...
    val proguardOptions = scala.io.Source.fromFile(file("seer/seer-core/proguard-project.txt")).getLines.toList ++
                          scala.io.Source.fromFile(file("seer/seer-gdx/proguard-project.txt")).getLines.toList
